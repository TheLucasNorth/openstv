...
def probDist(n, low = 0.8, high=1.0):
  ...
  while not (e < eHigh and e > eLow):
    angles = []
    for i in range(n-1):
      angles.append(random.uniform(0, old_div(math.pi,2)))
...
###
  ...
  def grid(self, n):
    ...
    for i in range(1, 2*n, 2):
      p = old_div(old_div(1.0*i,2),n) # P within a disk of radius R
      r.append( math.sqrt(-2*math.log(1-p)) )

    # compute the centers of bins in angle
    a = []
    for i in range(n):
      a.append( old_div(2*math.pi*i,n))

    # convert these to x,y coords
    self.bins = []
    for i in range(n):
      for j in range(n):
        x = r[i]*math.cos(a[j] + old_div((i%2)*math.pi,n))
        y = r[i]*math.sin(a[j] + old_div((i%2)*math.pi,n))
  ...
  def display(self, ybins=10, cutoff=1.0, v=""):
    ...
    for c in self.c:
      x = int(round(old_div(self.p[c][0]*xbins,cutoff)))
      x = max(x, -xbins)
      x = min(x, xbins)
      x += xbins

      y = -int(round(old_div(self.p[c][1]*ybins,cutoff)))
      y = max(y, -ybins)
      y = min(y, ybins)
      y += ybins
    ...
    if v != "":
      x = int(round(old_div(v[0]*xbins,cutoff)))
      x = max(x, -xbins)
      x = min(x, xbins)
      x += xbins

      y = -int(round(old_div(v[1]*ybins,cutoff)))
      y = max(y, -ybins)
      y = min(y, ybins)
      y += ybins
...
"""
  ...
  def load(self, fName):
    ...
    if x == "NOTA = 1":
      ...
      raise_(RuntimeError, "Bad format in bigram file: %s" % (x))
    ...
    for i in range(n):
      ...
      if y is None:
        raise_(RuntimeError, "Bad format in bigram file %s: %s" % (fName, x))
    ...
    if self.NOTA:
      for c in self.c:
        ...
        if s > 1:
          raise_(RuntimeError, "Conditional probabilities for %s sum to greater than 1." % (c))
        self.p2[c]["NOTA"] = 1 - s
    ...
    for c in self.c:
      s = sum(self.p2[c].values())
      if abs(s-1) > eps:
        raise_(RuntimeError, "Conditional probabilities for %s must sum to exactly 1." % (c))
  ...
  def estimate(self, ballots, levels=1, verbose=0):
    ...
    for c in self.c:
      self.p1[c] = old_div(1.0*count[c],ballots.n)
    for i in range(len(self.c)):
      self.plength[i] = old_div(1.0*countlength[i],ballots.n)
    ...
    while(1):
      ...
      for level in range(levels):
        for c in list(par_count[level].keys()):
          for prev in list(par_count[level][c].keys()):
            ...
            assert(sp < 1.0)
            for d in missing:
              count[c][d] += old_div(1.0 * self.p2[c][d] * sc, (1 - sp))
      ...
      for c in self.c:
        ...
        for d in list(p2[c].keys()):
          p2[c][d] = old_div(1.0 * count[c][d], s)
...
###
  ...
  def KLDist(self, b, r=1):
    ...
    for i in range(len(count1)):
      if count1[i] > 0:
        I += old_div(count1[i],s1) * math.log( old_div((old_div(count1[i],s1)), (old_div(count2[i],s2))) )
    return I
