...
"""
  ...
  def preCount(self):
    ...
    self.surplusLimit = old_div(self.p, 10000)	# 0.0001 per clause 13
    # initialize PRNG per clause 42
  ...
  def updateCount(self, tree=None, remainder=None):
    ...
    for c in list(tree.keys()):
      ...
      if method == "hill":
        ...
        self.count[self.R][c] += old_div(rrr * self.keepFactor[self.R][c] * tree[c]["n"], self.p)
        rrr = old_div(rrr * (self.p - self.keepFactor[self.R][c]), self.p)
